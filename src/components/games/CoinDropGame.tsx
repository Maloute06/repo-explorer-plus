  ...
  return (
    <div className="panel p-4 sm:p-6">
      <div className="relative aspect-4/3 w-full overflow-hidden rounded-xl border border-border bg-background/70">
        {Array.from({ length: ROWS }).map((_, r) => (
          ...
          >
            {Array.from({ length: 7 + (r % 2) }).map((_, c) => (
              <span key={c} className="size-1.5 rounded-full bg-gold/70" />
            ))}
          </div>
        ))}
        {sim.visual.map((coin) => {
        ...
        <div className="absolute inset-x-0 bottom-0 flex">
          {SLOTS.map((v, i) => (
            <div
              key={i}
              className="flex-1 border-t border-border py-1 text-center font-mono text-[10px] text-gold sm:text-xs"
              style={{ background: `oklch(0.828 0.14 88 / ${0.08 + v / 8000})` }}
            >
              {v}
            </div>
          ))}
      ...
      <CrowdNote total={players.length} shown={sim.visual.length}>
        Machine à sous géante — les meilleurs gains atterrissent tout en bas.
      </CrowdNote>
    </div>
  );
}
